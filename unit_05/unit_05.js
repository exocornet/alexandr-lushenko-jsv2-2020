...
function t10() {
  ...
  for (let i = oldYear; i <= newYear; i += 2) {
    divOut10.innerHTML += `${i} `;
  }
}
...
function t11() {
  ...
  for (let i = 0; i < div11.length; i++) {
    divOut11.innerHTML += div11[i].textContent + ' ';
  }
}
...
function t12() {
  ...
  for (let i = 0; i < div12.length; i++) {
    div12[i].style.background = (div12[i].style.background == 'orange') ? 'white' : 'orange';
  }
}
...
function t14() {
  ...
  for (let i = 0; i < i14.length; i++) {
    if(i14[i].checked){
      divOut14.innerHTML = i14[i].value;
    }
  }
}
