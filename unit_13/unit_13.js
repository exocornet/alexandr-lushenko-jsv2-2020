...
function f14() {
  ...
  let div141 = '';
  for (let key in a14) {
    div14[div14.length] = a14[key][0];
  }
  for (let i = 0; i < div14.length; i++) {
    div141 += div14[i] + ' ';
  }
  out14.innerHTML = div141;
}
...
function f17() {
  let div17 = '';
  for (let key1 in a17) {
    for (let key2 in a17[key1]) {
      if (key2 === 'name' && a17[key1]['age'] > 26) {
        div17 += a17[key1][key2] + ' ';
      }
    }
  }
  out17.innerHTML = div17;
}
...
function f18() {
  ...
  for (let key in a18) {
    ...
    if (i18.value == key) {
      ...
      div18 = 'Такой ветки не существуеет';
    }
  }
...
let a19 = {
    "red": ['Akademmistechko', 'Nyvky', 'Universytet', 'Lisova', 12],
    "blue": ['Minska', 'Obolon', 'Pochaina', 'Holosiivska'],
    "green": ['Syrets', 'Zoloti Vorota', 'Klovska', 'Vidubichi']
}
...
function f19() {
  ...
  for (let i = 0; i < i191.length; i++) {
    if (i == 0) {
      s += i191[i].toUpperCase();
    } else {
      s += i191[i];
    }
  }
